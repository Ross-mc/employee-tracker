...
const addPosition = position => {
    ...
    return new Promise((resolve, reject) => {
        connection.query("INSERT INTO position SET ?", position, (err, res) => {
            if (err) throw reject(err);
            console.log(`${title} successfully added to database!`);
            resolve();
        })
    })
}

addPosition({title: 'testing', salary: 50000, department_id: 4})
